...
function generateCloudConfig ( cos, node ) {
    ...
    switch ( node.role ) {
        ...
        case roles[ 1 ]:
                ...
                cloudConfig.configure( 'etcd2', {
                    ...
                    "initial-cluster-state": nodesIds.length > 0 ? "existing" : "new",
    ...
    cloudConfig.configure( 'fleet', fleetCfg );
    ...
    // worker role
    if ( node.role === roles[ 0 ] && nodesIds.length > 0 ) {
        fleetCfg.etcd_servers = etcd_endpoints;

        cloudConfig.configure( 'locksmith', {
