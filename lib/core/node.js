...
var Node = Class.extend({
    ...
    initialize: function ( data, cos ) {
        ...
        if ( Util.is.Object( data ) ) {
            for( var i in data ) {
                if( data.hasOwnProperty( i ) && typeof data !== 'undefined' ) {
                    node[ i ] = data[ i ];
                }
            }
        }
        ...
        if ( node.account.initializeNode ) {
            node.account.initializeNode( node );
        }

        if ( ! node.ip && node.host ) {
            debug( 'ensuring that we have an ip!!' );
            node.ip = dns.resolve( node.host );
        }
    ...
    syncFirewallRules: function () {
        return this.account.nodeSyncFirewallRules();
    },
    ...
    healthCheck: function () {
        ...
        var beforeChecking = JSON.stringify( node.firewallRules );

        return Promise.cast()
        .then(function () {
            debug( 'setting up ssh firewall rule' );
        ...
        .then(function () {
            debug( 'setting up etcd2 firewall rules' );
        ...
        .then(function () {
            debug( 'setting up flannel firewall rules' );
        ...
        .then(function () {
            debug( 'checking if firewall rules needs syncing' );

            if ( beforeChecking !== JSON.stringify( node.firewallRules ) ) {
                debug( 'WARNING: firewall rules were not as it should be' );
                debug( 'syncing them' );
